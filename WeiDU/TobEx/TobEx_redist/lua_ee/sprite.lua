...
function sprite_getcurrent()
  ...
  local _f = pushcfunction(func_CArea_GetCursorTarget)
  local _enum = u2n(_area:stdcall(_f))
...
function sprite_gettype(_obj)
  ...
  if _type == 0 then
    ...
    if _vtable == vtable_CChunks then
      return "sprite_chunks"
    elseif _vtable == vtable_CSparkle then
      return "sprite_sparkle"
    elseif _vtable == vtable_CProjectileBAM then
      return "sprite_projbam"
    elseif _vtable == vtable_CVisualEffectVidCell then
      return "sprite_vvc"
    elseif _vtable == vtable_CVisualEffect then
      return "sprite_vef"
    elseif _vtable == vtable_CCastingGlow then
      return "sprite_castglow"
    elseif _vtable == vtable_CSpellHit then
